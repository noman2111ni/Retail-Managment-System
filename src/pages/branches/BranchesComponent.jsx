...
import { FiSearch, FiPlus, FiEdit2, FiTrash2, FiMapPin, FiPhone, FiChevronLeft, FiChevronRight, FiMoreHorizontal } from 'react-icons/fi';
    ...
    <div className="min-h-screen bg-white dark:bg-gray-900 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {/* Branches */}
          <div className="dark:bg-gray-800 rounded-lg shadow p-6 hover:shadow-md flex items-center">
        ...
        {/* Branches Table */}
        <div className="dark:bg-gray-800 shadow overflow-hidden rounded-lg">
            ...
            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
                {currentBranches.map((branch) => (
                  <tr
                    key={branch.id}
                    className="hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors duration-200 cursor-pointer capitalize"
                    onClick={() => navigate(`/branches/${branch.id}`)}
                  >
                    <td className="px-6 py-4 whitespace-nowrap flex items-center">
                      <div className="flex-shrink-0 h-10 w-10 bg-yellow-100 dark:bg-yellow-900 rounded-full flex items-center justify-center shadow-sm">
                        <FiMapPin className="h-5 w-5 text-yellow-600 dark:text-yellow-300" />
                      </div>
                      <div className="ml-4">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">{branch.location || "-"}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">{branch.phone || "-"}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 dark:text-gray-300">{(branch.email || "-").slice(0, 12)}</td>
            ...
            <div className="bg-white dark:bg-gray-800 px-4 py-4 flex items-center justify-end border-t border-gray-200 dark:border-gray-700 sm:px-6">
              <div className="flex items-center justify-end space-x-1">
                <button
                  ...
                  className="flex items-center justify-center w-9 h-9 rounded-full border border-gray-300 dark:border-gray-700 text-gray-500 dark:text-gray-400 bg-gradient-to-r from-yellow-400 to-yellow-500 dark:from-yellow-600 dark:to-yellow-700 hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed transition"
                >
                  <FiChevronLeft />
                </button>
                ...
                {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                  <button
                    ...
                    className={`w-9 h-9 rounded-full text-sm font-medium transition ${
                      currentPage === page
                        ? "bg-gradient-to-r from-yellow-400 to-yellow-500 dark:from-yellow-600 dark:to-yellow-700 text-gray-900 dark:text-gray-100 shadow-md"
                        : "bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:bg-yellow-100 dark:hover:bg-yellow-900/20"
                    }`}
                  >
                    {page}
                  </button>
